...
export default {
  ...
  methods: {
    ...
    emailHint (row) { const o = this.payload(row).orderDraft || {}; const state = this.isEmailChecking(row) ? 'Проверяем письмо, результат появится автоматически' : row.queueState === 'check_failed' ? 'Проверка не завершилась — письмо сохранено' : ''; return [state, o.counterpartyName, this.formatDateTime(o.pickupAt), [o.fromPoint, o.toPoint].filter(Boolean).join(' → ')].filter(Boolean).join(' · ') },
    formatDateTime (value) { if (!value) return ''; const date = new Date(value); if (Number.isNaN(date.getTime())) return String(value); return date.toLocaleString('ru-RU', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', timeZone: 'UTC' }) },
...
<style scoped>
.work-queue{background:#fff;border:1px solid #dfe5ee;border-radius: 8px;overflow:hidden}.work-list{display:flex;flex-direction:column}.work-row{display:grid;grid-template-columns:90px minmax(0,1fr) auto;gap:16px;align-items:center;width:100%;padding:15px 18px;border:0;border-bottom:1px solid #edf1f6;background:#fff;color:inherit;text-align:left;text-decoration:none;cursor:pointer}.work-row:hover{background:#f8fafc}.work-row:disabled{cursor:wait;opacity:.72}.work-row__source{color:#65738a;font-size:12px;font-weight:800;text-transform:uppercase}.work-row__main strong,.work-row__main small{display:block}.work-row__main small{margin-top:4px;color:#65738a}.work-row__action{color:#20355f;font-weight:800}.work-empty{display:flex;flex-direction:column;gap:8px;align-items:center;padding:48px 20px;color:#65738a;text-align:center}.work-empty--error{color:#991b1b}@media(max-width:700px){.work-row{grid-template-columns:1fr;gap:6px}.work-row__action{margin-top:4px}}
</style>
