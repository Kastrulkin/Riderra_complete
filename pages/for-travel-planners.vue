<template>
  <div>
    ...
    <section class="site-section site-section--pf for-planners-section">
      <div class="container">
        <!-- Hero -->
        ...
        <div class="planners-section">
          ...
          <div class="economics-list">
            <div class="economics-item">
              <strong>{{ t.economics.fleetPrice }}</strong>
              <span>{{ t.economics.fleetPriceDesc }}</span>
            </div>
            <div class="economics-item">
              <strong>{{ t.economics.serviceFee }}</strong>
              <span>{{ t.economics.serviceFeeDesc }}</span>
            </div>
            <div class="economics-item">
              <strong>{{ t.economics.agencyMarkup }}</strong>
              <span>{{ t.economics.agencyMarkupDesc }}</span>
            </div>
            <div class="economics-item">
              <strong>{{ t.economics.clientPrice }}</strong>
              <span>{{ t.economics.clientPriceDesc }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="planners-section">
          ...
          <div class="features-grid">
            <div class="feature-item" v-for="(feature, index) in t.features" :key="index">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M20 6L9 17L4 12" stroke="#1a237e" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
              <span>{{ feature }}</span>
            </div>
          </div>
        ...
        <div class="planners-section">
          <h2>{{ t.faqTitle }}</h2>
          <div class="faq-list">
            <div class="faq-item" v-for="(faq, index) in t.faq" :key="index">
              <h3>{{ faq.question }}</h3>
              <p>{{ faq.answer }}</p>
            </div>
          </div>
...
export default {
  ...
  computed: {
    ...
    t() {
      const dict = {
        ru: {
          heroTitle: 'Международные трансферы для турагентов без лишних посредников',
          heroSubtitle: 'Обычно при заказе трансфера есть два неудобных пути: либо переплачивать цепочке посредников, либо каждый раз искать и проверять перевозчика в новой стране. Riderra даёт доступ к проверенным местным автопаркам через один канал и сопровождает заказ.',
          ctaPrimary: 'Обсудить доступ агента',
          ctaSecondary: 'Создать запрос трансфера',
          problemTitle: 'Либо переплата посредникам, либо поиск перевозчика в каждой стране',
          problemText: 'При заказе международного трансфера обычно возникает одна из двух проблем: либо цена растёт из-за цепочки из 3–5 посредников, либо турагенту приходится самому искать и проверять перевозчика в каждой новой стране. В обоих случаях ответственность перед клиентом остаётся на вас.',
          solutionTitle: 'Один партнёр вместо длинной цепочки посредников',
          solutionText: 'Riderra даёт турагентам доступ к проверенным местным автопаркам и ведёт заказ в одном процессе: подтверждает цену и доступность, помогает с ваучером, сопровождает изменения и поддерживает клиента по ходу поездки.',
          economicsTitle: 'Прозрачная экономика для агентов',
          economics: {
            fleetPrice: 'Нетто-цена местного автопарка',
            fleetPriceDesc: 'Оплачивается местному перевозчику',
            serviceFee: 'Комиссия Riderra',
            serviceFeeDesc: 'Охватывает координацию, поддержку и операционный контроль',
            agencyMarkup: 'Ваша маржа',
            agencyMarkupDesc: 'Устанавливается вами',
            clientPrice: 'Цена для клиента',
            clientPriceDesc: 'Показывается вашему клиенту'
          },
          featuresTitle: 'Что получает турагент',
          features: [
            'Нетто-цены от местных автопарков',
            'Возможность добавить свою маржу',
            'Готовый PDF-ваучер для клиента',
            'Сообщение для пассажира в WhatsApp',
            'Ссылка на статус заявки',
            'Отслеживание рейсов и правила ожидания',
            'Ежемесячный счёт-фактура',
            'Ссылка для клиента с белым брендом',
            'Быстрое повторное бронирование'
          ],
          howItWorksTitle: 'Как это работает',
          steps: [
            { title: '1. Отправьте детали поездки', desc: 'Маршрут, дата, номер рейса, пассажиры, багаж и особые пожелания.' },
            { title: '2. Получите проверенное предложение', desc: 'Riderra уточняет цену и доступность у местного автопарка до финального бронирования.' },
            { title: '3. Отправьте клиенту готовый ваучер', desc: 'Вы добавляете свою маржу и отправляете клиенту готовый документ.' },
            { title: '4. Мы сопровождаем заказ', desc: 'Вы видите статус заявки, а Riderra помогает, если меняются детали поездки.' }
          ],
          faqTitle: 'Часто задаваемые вопросы',
          faq: [
            { question: 'Увидит ли пассажир мою нетто-цену?', answer: 'Нет. Клиент получает готовый ваучер без вашей внутренней цены.' },
            { question: 'Кто координирует работу с местным автопарком?', answer: 'Riderra ведёт коммуникацию с местным автопарком, подтверждает детали заказа и помогает при изменениях.' },
            { question: 'Могу ли я добавить свою маржу?', answer: 'Да. Вы сами определяете цену для клиента поверх нетто-цены и комиссии Riderra.' },
            ...
            { question: 'Что происходит, если рейс задерживается?', answer: 'Riderra отслеживает рейс и координирует встречу с учётом правил ожидания.' }
          ],
        ...
        en: {
          ...
          economics: {
            ...
            fleetPriceDesc: 'paid to the local operator',
            serviceFee: 'Riderra service fee',
            serviceFeeDesc: 'covers coordination, support and operational control',
            agencyMarkup: 'Your agency markup',
            agencyMarkupDesc: 'set by you',
