...
const { VENDOR_WIKI_SECTIONS } = require('../data/vendorWiki')
const {
  VENDOR_LANGUAGES,
  ...
  partnersPath,
  wikiPath
} = require('../data/vendorWikiLocales')

function renderBlock(block, ui) {
  ...
  const rowHeadings = Array.isArray(block.headings)
    ? block.headings
    : block.title === 'Trip status sequence'
      ? [ui.rowStatus, ui.rowMeaning, ui.rowWhen]
      : [ui.rowClass, ui.rowCapacity, ui.rowExamples]
  const rows = Array.isArray(block.rows) ? `<div class="table-wrap"><table><thead><tr>${rowHeadings.map((heading) => `<th>${escapeHtml(heading)}</th>`).join('')}</tr></thead><tbody>${block.rows.map((row) => `<tr>${row.map((cell) => cell && typeof cell === 'object' && cell.href ? `<td><a class="table-link" href="${escapeHtml(cell.href)}">${escapeHtml(cell.label || cell.text || '')}</a></td>` : `<td>${escapeHtml(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>` : ''
  const figure = block.figure
    ? `<figure class="wiki-figure"><a href="${escapeHtml(block.figure.src)}" target="_blank" rel="noopener"><img src="${escapeHtml(block.figure.src)}" alt="${escapeHtml(block.figure.alt || '')}" loading="lazy" decoding="async"></a><figcaption>${escapeHtml(block.figure.caption || '')}${block.figure.sourceHref ? ` <a href="${escapeHtml(block.figure.sourceHref)}" target="_blank" rel="noopener noreferrer">${escapeHtml(block.figure.sourceLabel || 'Source')}</a>` : ''}</figcaption></figure>`
    : ''
...
function renderVendorWikiHtml(requestedLanguage = 'en', options = {}) {
  ...
  const sections = wikiSections.map((section, index) => {
    const articles = section.articles.map((article) => {
      const searchText = `${article.title} ${article.intro} ${article.blocks.map((block) => `${block.title || ''} ${block.body || ''} ${(block.items || []).join(' ')} ${(block.rows || []).flat().map((cell) => cell && typeof cell === 'object' ? cell.label || cell.text || '' : cell).join(' ')} ${block.figure?.caption || ''}`).join(' ')}`.toLowerCase()
      return `<article id="${escapeHtml(article.id)}" class="wiki-article" data-search="${escapeHtml(searchText)}"><div class="article-heading"><a class="anchor" href="#${escapeHtml(article.id)}" aria-label="${escapeHtml(article.title)}">#</a><div><h2>${escapeHtml(article.title)}</h2><p>${escapeHtml(article.intro)}</p></div></div>${article.blocks.map((block) => renderBlock(block, ui)).join('')}</article>`
    }).join('')
...
module.exports = { renderVendorWikiHtml }
