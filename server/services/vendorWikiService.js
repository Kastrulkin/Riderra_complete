...
function renderVendorWikiHtml(requestedLanguage = 'en', options = {}) {
  ...
  const sections = wikiSections.map((section, index) => {
    const articles = section.articles.map((article) => {
      const searchText = `${article.title} ${article.intro} ${article.blocks.map((block) => `${block.title || ''} ${block.body || ''} ${(block.items || []).join(' ')} ${(block.rows || []).flat().join(' ')} ${block.figure?.caption || ''}`).join(' ')}`.toLowerCase()
