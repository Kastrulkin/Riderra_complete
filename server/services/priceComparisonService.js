...
function externalVehicleMatches(adapterKey, externalVehicleKey, riderraVehicleType) {
  ...
  if (adapterKey === 'talixo') {
    const external = normalizeTextKey(externalVehicleKey).replace(/\s+/g, '_')
    const internal = normalizeTextKey(riderraVehicleType)
    if (external === 'economy') return /(standard class car|standard sedan|sedan|saloon)/i.test(internal) && !/(executive|business|first|electric|mini.?van|mpv|bus)/i.test(internal)
    if (external === 'business') return /(business class car|business sedan|executive)/i.test(internal) && !/(van|mpv)/i.test(internal)
    if (external === 'first_class') return /(first class|luxury)/i.test(internal) && !/(van|mpv)/i.test(internal)
    const internalCapacity = Number(internal.match(/(\d+)\s*pax/)?.[1])
  ...
  if (adapterKey === 'dottransfers') {
    const external = normalizeTextKey(externalVehicleKey).replace(/\s+/g, '_')
    const internal = normalizeTextKey(riderraVehicleType)
    const internalCapacity = Number(internal.match(/(\d+)\s*pax/)?.[1])
    const match = external.match(/^(standard|business|first_class)_(sedan|van|minibus)_(\d+)$/)
    if (!match) return false
    const [, category, family, seats] = match
    const capacity = Number(seats)
    if (family === 'sedan') {
      if (category === 'standard') return /(standard class car|standard sedan|sedan|saloon)/i.test(internal) && !/(executive|business|first|electric|mini.?van|mpv|bus)/i.test(internal)
      if (category === 'business') return /(business class car|business sedan|executive)/i.test(internal) && !/(van|mpv)/i.test(internal)
      return /(first class|luxury)/i.test(internal) && !/(van|mpv)/i.test(internal)
    }
    if (family === 'van') {
      if (category === 'standard') {
        if (capacity === 4) return /standard mpv/i.test(internal) && (!Number.isFinite(internalCapacity) || internalCapacity === 4)
        return /standard mini.?van/i.test(internal) && internalCapacity === capacity
      }
      if (category === 'business') return /(businessvan|business.*van|executive.*van)/i.test(internal) && internalCapacity === capacity
  ...
  if (adapterKey === 'mytransfers') {
    const external = normalizeTextKey(externalVehicleKey).replace(/\s+/g, '_')
    const internal = normalizeTextKey(riderraVehicleType)
    const internalCapacity = Number(internal.match(/(\d+)\s*pax/)?.[1])
    const match = external.match(/^(.+)_(\d+)$/)
    const family = match?.[1] || external
    const capacity = Number(match?.[2])
    ...
    if (family === 'standard_mpv') return /standard mpv/i.test(internal) && (!Number.isFinite(internalCapacity) || internalCapacity === capacity)
    if (family === 'standard_minivan') return /standard mini.?van/i.test(internal) && internalCapacity === capacity
    if (family === 'businessvan') return /(businessvan|business.*van|executive.*van)/i.test(internal) && internalCapacity === capacity
    if (family === 'standard_minibus') return /standard mini.?bus/i.test(internal) && internalCapacity === capacity
    return false
  }
...
async function resolveTransferzBenchmarkPlace({ prisma, tenantId, source, adapter, zoneName }) {
  ...
  const direct = typeof adapter.createBenchmarkPlace === 'function' ? adapter.createBenchmarkPlace(point) : null
  const candidates = direct ? [direct] : await adapter.resolvePlace(benchmarkAddress)
  const selected = direct || selectPlaceCandidate(benchmarkAddress, candidates)
  ...
  return { ok: true, id: selected.id, label: selected.label, mapping, benchmarkPointId: point.id, externalRequest: !direct }
}
