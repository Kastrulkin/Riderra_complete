...
async function canonicalRegistryFromActiveSheet(source) {
  ...
  const valuesUrl = `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${encodeURIComponent(range)}`
...
async function main() {
  ...
  const tenant = await prisma.tenant.findFirst({ where: { code: 'riderra' }, select: { id: true, code: true } })
  if (!tenant) throw new Error('Riderra tenant not found')
  const activeSource = await prisma.sheetSource.findFirst({
  ...
  const sources = await prisma.sheetSource.findMany({
    where: { tenantId: tenant.id, lastSyncStatus: { not: 'superseded' } },
    orderBy: [{ monthLabel: 'asc' }, { updatedAt: 'desc' }],
    select: { id: true, monthLabel: true, name: true }
  })
  const totals = {
    apply: args.apply,
  ...
  for (const source of sources) {
    const snapshots = await prisma.orderSourceSnapshot.findMany({
      ...
      select: {
        sourceRow: true,
        rawPayload: true,
        order: { select: { id: true, driverNameRaw: true, sourceComment: true, manualOverridesJson: true } }
      }
    })
    ...
    for (const snapshot of snapshots) {
      ...
      totals.rows++
      totals.orders++
      if (manualDriverOverride(snapshot.order)) {
        totals.manualOverrides++
        continue
      }
      const sourceValue = exactDriverValue(rawPayload(snapshot))
      const resolution = resolveCanonicalDriverName(sourceValue, registry)
      ...
      totals.wouldUpdate++
      if (!nextValue && currentValue && currentValue === String(snapshot.order.sourceComment || '').trim()) {
        totals.clearedInvalidComments++
      }
      if (args.apply) {
