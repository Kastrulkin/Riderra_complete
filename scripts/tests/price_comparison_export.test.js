...
function fixtureRun() {
  return {
    ...
    pricingPolicyJson: JSON.stringify({ type: 'client_commission', commissionPercent: 30 }),
    ...
    quotes: [{
      ...
      result: { targetPrice: 140, opportunityGapAbs: 40, opportunityGapPct: 28.57, status: 'opportunity' }
    }, {
...
test('Excel export contains required sheets and auditable formulas', async () => {
  ...
  assert.match(workbook.getWorksheet('All results').getCell('I2').value.formula, /H2/)
  assert.match(workbook.getWorksheet('All results').getCell('L2').value.formula, /F2<I2/)
