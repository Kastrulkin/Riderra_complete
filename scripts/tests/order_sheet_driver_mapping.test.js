...
test('Google Sheet plural driver column wins over a comment mentioning a driver', () => {
  ...
  const row = { 'Водители': 'Riga Taxi', 'Комментарий (то, что было в скобках, водитель вендора, машина и т.д.)': 'У нас маленькая собака 2 кг в переноске' }
  assert.equal(rawFirst(row, ['driver', 'водитель']), 'Riga Taxi')
})

test('an empty driver column does not turn an operational comment into a driver', () => {
  const rawFirst = loadProductionRawFirst()
  const row = { 'Водители': '', 'Комментарий (то, что было в скобках, водитель вендора, машина и т.д.)': 'Уточнить детское кресло' }
...
test('driver typo resolves to the unique canonical performer from the technical sheet', () => {
  const registry = buildDriverCanonicalRegistry(['Цепенников Сергей', 'Кувшинов Сергей', 'Riga Taxi'])
  assert.deepEqual(resolveCanonicalDriverName('Цепеников Сергей', registry), { value: 'Цепенников Сергей', matched: true, method: 'fuzzy', distance: 1 })
})
