...
test('fetches a quote with exact public parameters and never posts a booking', async () => {
  ...
  const catalog = await adapter.fetchCatalogRoute({ route: adapter.routes[0], serviceAt: new Date('2026-08-19T12:00:00.000Z'), currencies: ['EUR', 'USD'], passengers: { adults: 1 } })
  assert.equal(catalog.matrices.length, 2)
  assert.equal(catalog.matrices[1].quotes[0].price, 47.38)
